...
class SquatDetector(BaseExercise):
    ...
    def process(self, landmarks): 
        left_knee_angle = self.calculate_angle(
            self.get_point(landmarks, self.LEFT_HIP),  # left hip
            self.get_point(landmarks, self.LEFT_KNEE),  # left knee
            self.get_point(landmarks, self.LEFT_ANKLE)  # left ankle 
        )
        right_knee_angle = self.calculate_angle(
            self.get_point(landmarks, self.RIGHT_HIP),  # right hip
            self.get_point(landmarks, self.RIGHT_KNEE),  # right knee
            self.get_point(landmarks, self.RIGHT_ANKLE)  # right ankle 
        )
        ...
        back_angle = self.calculate_angle(
            self.get_point(landmarks, shoulder_idx),  # shoulder
            self.get_point(landmarks, hip_idx),  # hip
            self.get_point(landmarks, knee_idx)  # knee
        )
        ...
        if key_landmark_visible:
            ...
            if self.stage == "down":
                depth_status="GOOD DEPTH"if knee_angle <= self.DEPTH_THRESHOLD else "TOO HIGH"
            elif self.stage == "up":
